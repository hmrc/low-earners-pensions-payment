...
class LowEarnersCalculationSpec extends SpecBase {
  "LowEarnersCalculation" - {
    "reads" - {
      "should return a JsError when reading from invalid JSON" in {
        val json: JsObject = JsObject.empty
        json.validate[LowEarnersCalculation] mustBe a[JsError]
      }
      ...
      "should return the expected model when reading from valid JSON" in {
        ...
        json.validate[LowEarnersCalculation] mustBe a[JsSuccess[_]]
        json.as[LowEarnersCalculation] mustBe calculation
      }
    ...
    "writes" - {
      "should produce the expected JSON" in {
        ...
        Json.toJson(calculation) mustBe json
      }
    }
  }
}
